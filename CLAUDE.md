...
## 🎯 當前任務(2026-09-03 對齊)
- **現況:52 張首頁卡、3 分類、13 個合輯(129 關)、sw v132**(0917 +約書亞・得地為業 3D 直達卡,立體圍棋,kind `board`)(0914 拔 SW 名單裡的 index.html;0905 HFP 機:+約書亞・得地為業直達卡,新 kind `board` 棋類對弈;v117~v119 是 0904 的三款小遊戲卡改指 CF)(`npm test` 印的數字以此為準;0903 對齊)。
...
- 09-03(agape250 機・Opus 5):✨ **大廳動態**(sw v115→v116;同信友火花 0902 那套「有節制的活」,使用者點名套到大廳):
  ...
  新檔 `src/motion.js`(進 SW CORE;一定排在 main.js 之後載入)。**總開關**=index.html head 一行掛 `html.js-motion`,所有 opacity:0 起始都掛在它底下
  ⇒ 減少動態 / 沒 JS / 舊瀏覽器全可見。★ IntersectionObserver rootMargin 上緣 100000px(捲過頭的區塊也算看過,否則永遠隱形)。
  驗:npm test + --offline 全綠、真瀏覽器 21 項(進場時序 / 捲到底無隱形 / 合輯內頁重畫全亮 / hover / 換句 / 拉幕與返回 / 減少動態 / 沒 JS)。
  部署走 `wrangler versions upload` 預覽 → 使用者拍板 → `versions deploy` 上線(⚠ CF 帳號 0903 起 ToS 審查中:不可建新 Worker)。
- 09-02 晚(HFP 機・Fable 5.1):✨ **毛毛大亂鬥再收進「憫安製作闖關合輯」**(`dd80bcd`,sw v114→v115):使用者拍板
  「毛毛大亂鬥是憫安的作品」⇒ 播放清單模式同一關兩處都有(街機合輯那張也掛 `credit: '製作:憫安'` 徽章);
  同時**從德義作品集撤下**(憫安作品不進德義作品集,作品集 check-sites 的 IGNORE 記著原因)。
  合輯關數 127→128。線上 `src/data.js` grep `animalbrawl3d-minan`=1、sw v115 驗過。
- 09-01(agape250 機):🐾 **毛毛大亂鬥**進**經典街機合輯**(第 5 張直達卡——
  艦隊**第一個用真物理引擎(Rapier)**的專案:物理布娃娃互推、把對手推下浮空台,
  四種動物/四視角/同機兩人或對電腦;純娛樂不接經文(0828 拍板)。
  repo `animalbrawl3d`(公開)/ CF Pages `hfpc-animalbrawl3d`.pages.dev;
  bytype +1 新 kind `brawl`(互推大亂鬥)——新 kind 不補家族表 smoke-test 會擋)+sw v114。
- 07-11(HFP 機):🏓 **3D 桌球**進休閒運動合輯(第十八彈——racket3d 第三運動 ?mode=pingpong;
  同輪全系列 3D 遊戲三視角→**五視角**(加左右側))+sw v51(v50=bytype 補 rhythm kind)。
- 07-11(HFP 機):🏮 **3D 功夫修行**進休閒運動合輯(第十七彈——打木人樁節奏模仿(非互毆),
  拳掌踢擋四鍵+評級;repo kungfutrain3d(私有)/hfpc-kungfutrain3d.netlify.app)+sw v49。
- 07-11(HFP 機):🤾 **3D 躲避球**進休閒運動合輯(第十六彈——接殺機制(圈變綠按 K=投球者出局)+
  溫柔出局(板凳揮手加油);每隊 3~6 人;repo dodgeball3d(私有)/hfpc-dodgeball3d.netlify.app)+sw v48。
