...
export const GROUPS = [
  {
    ...
    kinds: [
      ...
      ['brawl', '互推大亂鬥', '物理布娃娃互推,把對手推下浮空台(無經文休閒・同機兩人)'],
    ],
  },
  {
    name: '🎵 節奏與音樂',
  ...
  },
  {
    ...
    kinds: [
      ...
      ['puzzle', '拆解解謎', '照次序一層一層拆下來,拆錯順序就卡住(純邏輯、不比手速)'],
      ['deduce', '看數字推理', '數字說周圍有幾個,靠它推出哪裡安全、哪裡不能點(純邏輯;可開「無猜」保證不用賭)'],
      ['geohunt', '走路尋回(戶外)', '帶著手機走到現場,走近了才收得到——腳走到才算(需大人陪同)'],
      ['board', '棋類對弈', '兩人輪流落子的棋,一步一步把地圍成自己的(可對電腦,也可親子對弈)'],
    ],
  },
  {
    name: '📖 複習與課堂',
