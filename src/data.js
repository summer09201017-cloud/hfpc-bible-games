...
export const JOURNEYS = [
  ...
  },
  {
    ...
    // ✅ 2026-07-26:牧者過審文案與神學語氣後點亮,可以推給老師上課用。
    // 07-25 晚加難度(使用者實測「太簡單」):幾何改由 layout.js 依「疊柱規格」算出來,遮擋由結構保證;
    // 青少年檔「會想 bot」卡死:站2 23%・站3 20%・站4 19%・站5 26%・站6 31%(第一站是教學關,故意不卡死)。
    // 07-26 美術:各材料照經文上色與形狀(撚的細麻/山羊毛/染紅公羊皮/海狗皮/包金板/純藍色的毯子;
    // ★院子是帶卯的銅座 出 27:10、帳幕是帶卯的銀座 出 26:19),剩最後一根橛子時那片板搖搖欲墜。
    // 地圖與年齡選單照 tsum 七款:年齡檔獨立第一畫面 + 🗺 地圖 + ★★★(不用提示不重來=三顆)。
    id: 'tabernacle-screw',
  ...
  },
  {
    ...
    url: 'https://hfpc-bible-games.summer09201017.workers.dev/bingo',
  },
...
export const COLLECTIONS = {
  ...
  minan: {
    ...
    desc:
      ...
      ' <a href="https://minan-portfolio.summer09201017.workers.dev/" target="_blank" rel="noopener">✨ 憫安作品集完整版(含成長時間線)→</a>',
    color: '#f0b840',
    emoji: '✨',
    items: [
      {
  ...
  goodshepherd: {
    ...
    desc:
      ...
      '重點不是「我抓得準」,是「牧人必尋回」。' +
      // ⚠ 這段會原樣印在畫面上(不經 Markdown 轉換)——強調一律用「」,絕不能寫 **粗體**,
      //   星號會直接被使用者看到(hook markdown-leak-guard #34 的病;0812 這裡自己踩了一次,
      //   因為那支 hook 只看 innerHTML 指派與含 HTML 標籤的字串,純資料字串不在它判準內)。
